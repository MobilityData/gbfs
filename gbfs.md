...
\- `vehicle_image`<br/>*(added in vXXX)* | OPTIONAL | URL | URL to an image that would assist the user in identifying the vehicle (e.g. logo, image of vehicle).<br /> Allowed formats: JPEG, PNG. Minimum resolution of 300 pixels per inch (ppi).
...
\- `current_fuel_percent` <br/>*(added in vXXX)*| OPTIONAL | Non-negative float | This value represents the current percentage, expressed from 0 to 1 of fuel or battery power remaining in the vehicle.
