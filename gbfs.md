...
\- `cargo_volume_capacity`<br/>*(added in vXXX)* | OPTIONAL | Non-negative integer | Cargo volume available in the vehicule, expressed in liters. For cars, it corresponds to the space between the boot floor, including the storage under the hatch, to the rear shelf in the trunk.
...
\- `current_fuel_percent` <br/>*(added in vXXX)*| OPTIONAL | Non-negative float | If the corresponding vehicle_type definition for this vehicle has a motor, then this field is required. This value represents the percentage, expressed from 0 to 1 of fuel or battery power remaining with the vehicle’s current charge or fuel level.
