...
`vehicles[].station_id` | Conditionally REQUIRED | ID | REQUIRED if the vehicle's station is predetermined (for example when the vehicle has to be returned to the same station from which it was initially rented). Identifier referencing the `station_id` field in [station_information.json](#station_informationjson).
