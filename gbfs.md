...
\- `car_type` | Optional | Enum | The type of car. <br /><br />Current valid issues are: <br /><ul><li>`ultra mini` _(e.g Smart Fortwo_)</li><li>`compact` _(e.g. Hyundai Accent)_ </li><li>`sedan` _(e.g. Chevrolet Malibu)_</li><li>`station-wagon` _(e.g. Subaru Outback)_ </li><li>`suv` _(e.g. Honda Pilot)_ </li><li>`minivan` _(e.g. Dodge Grand Caravan)_ </li><li>`pickup truck` _(e.g. Chevrolet Ram)_</li><li>`cargo van` _(Ford Transit 250)_ </li><li>`other`
\- `seating_capacity` | Optional | Non-negative integer | The number of riders the vehicle can legally accommodate.
\- `return_type` | Optional | Enum | The return conditions of the vehicle.<br /><br />Current valid values are:<br /><br /><ul><li>`roundtrip` _(Vehicle must be returned to pickup location)_</li><li>`one-way` _(Vehicle does not need to be returned to pickup location)_</li><li>`flexible_roundtrip` _(Vehicle must be returned to a predetermined location which can be different from the pickup location)_
...
\- `station_types` | Optional | Array | Types of stations that this vehicle type can be returned to.<br /><br />Current valid issues are:<br /><ul><li>`parking_lot` _(Off-street parking lot)_</li><li>`street_parking` _(Curbside parking)_</li><li>`underground_parking` _(Parking that is below street level, station may be non-communicating)_</li><li>`sidewalk_parking` _(Park vehicle on sidewalk)_</li><li>`other`
\- `vehicle_desc` | Optional | String | Description of the vehicle that provides useful, quality infromation. Do not simply duplicate the information already specified.
  ...
  "data": {
    "vehicle_types": [
      ...
      },
      {
        ...
        "station_types": ["parking_lot", "street_parking"],
...
\-&nbsp;`access_methods` | Optional |Array | Access methods at this station to unlock or access a vehicle.<br /><br />Current valid values are:<br /><ul><li>`key` _(e.g. operator issues vehicle key / fob / card)_</li><li>`transitcard` _(Non-operator issued transit pass this is linked to operator account)_</li><li>`dropbox` _(e.g. must retrieve key from dropbox in underground parking garage due to station being non-communicating)_</li><li>`code` _(e.g. alpha-numeric code provided on app upon booking, or QR code)_</li><li>`attendant` _(e.g. must retrieve key from an employee at the station)_
...
\-&nbsp;`charging_capacity` | Optional | Non-negative integer | Number of spots at this station that have charging ports for electric vehicles.<br /><br />If this field is empty, it means the station does not support charging electric vehicles.
\-&nbsp;`parking_hoop_access` | Optional | Array | Array of methods to unlock a parking hoop at the station to return the vehicle. If this field is empty, it means there is no parking hoop.<br /><br /><ul><li>`key`</li><li>`phone`</li><li>`code`</li><li>`remote`
\-&nbsp;`step_free_access` | Optional | Boolean | Can this station be accessed without having to take stairs? If this field is empty it means this station is not step free.<br /><br />`true` - This station ahs step free access.<br />`false` - This station does not have step free access.
...
\-&nbsp;`station_hours` | Optional | Array | Hours of operation at this station.<br /><br />If this field is empty, it means the station is always in operation.
&emsp;\-&nbsp;`days` | Optional | Array | An array of abbreviations (first 3 letters) of English names of the days of the week for which this station is in operation (e.g. ["mon", "tue', "wed", "thu", "fri", "sat", "sun"]).<br /><br />If this field is empty, it means the station hours do not vary by day of the week.
&emsp;\-&nbsp;`start_time` | Conditionally Required | Time | Start time for the hours of operation of the station in the time zone indicated in `system_information.json`.<br /><br />If this field is empty, it means the station hours do not vary by time. <br /><br />Conditionally Required: Required if `station_information.end_time` is provided. Optional otherwise.
&emsp;\-&nbsp;`end_time` | Conditionally Required | Time | End time for the hours of operation of the station in the time zone indicated in `system_information.json`.<br /><br />If this field is empty, it means the station hours do not vary by time. <br /><br />Conditionally Required: Required if `station_information.start_time` is provided. Optional otherwise.
